...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        if (!pdfFile || !coordsString) {
            return res.status(400).json({ message: 'Missing PDF file or coordinates.' });
        }
        ...
        try {
            ...
            if (typeof coords.page !== 'number' || typeof coords.xRatio !== 'number' || typeof coords.yRatio !== 'number' || typeof coords.width !== 'number' || typeof coords.height !== 'number') {
        ...
        if (error instanceof Error) {
            ...
            if (persistentPdfPath && fs.existsSync(persistentPdfPath)) {
                try {
                    ...
                    console.error(`[generate-sign-link] Error cleaning up PDF ${persistentPdfPath}:`, cleanupError);
                }
            }
            if (tempPdfPath && fs.existsSync(tempPdfPath)) {
                try {
                    ...
                    console.error(`[generate-sign-link] Error cleaning up temp PDF ${tempPdfPath}:`, cleanupError);
                }
            }
            ...
            console.error('[generate-sign-link] Unknown error processing PDF upload:', error);
            res.status(500).json({ message: 'An unknown error occurred during PDF upload.' });
        }
    }
}
