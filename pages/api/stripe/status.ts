...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // check Stripe connection status," indistinguishable in logs/responses from a genuine outage.
  if (!process.env.STRIPE_SECRET_KEY) {
    return res.status(503).json({ connected: false, message: "Billing is not configured." })
  }
  ...
  try {
    ...
    handleApiError(res, "/api/stripe/status", error, "Failed to check Stripe connection status.")
  }
}
