...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // This route reads/writes exactly one client row, so a staff member who hasn't picked one must
  // be told to, not silently land on Vierra's own company — see resolveExplicitTargetCompanyId's
  // doc comment (lib/api/targetCompany.ts) for why that differs from resolveTargetCompanyId.
