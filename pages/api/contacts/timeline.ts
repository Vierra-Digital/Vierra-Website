...
export default withAuth(
  async (req, res, session) => {
    ...
    const [outboundEvents, campaignEvents, bookingEvents] = await Promise.all([
      ...
      (async (): Promise<TimelineEvent[]> => {
        try {
          ...
          for (const m of outbound) {
            evs.push({ type: "sent", at: m.created_at.toISOString(), label: "Email sent", detail: m.subject || "(no subject)" });
    ...
    const events: TimelineEvent[] = [...outboundEvents, ...campaignEvents, ...bookingEvents];
