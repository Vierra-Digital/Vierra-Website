...
// Mirrors app/social-connect-failed/page.tsx's REASON_MESSAGES — kept in sync manually since one
// lives in the popup (server-rendered redirect target) and the other in the opener tab.
const SOCIAL_FAILURE_MESSAGES: Record<string, string> = {
...
export default function SessionQuestionnaire({ initialSession }: { initialSession: ClientSessionData }) {
  ...
  const [liLabel, setLiLabel] = useState<string | null>(null);
  const [gaLabel, setGaLabel] = useState<string | null>(null);
  const [socialLoading, setSocialLoading] = useState(false);
  const [socialDisconnecting, setSocialDisconnecting] = useState<"facebook" | "linkedin" | "googleads" | null>(null);
  const [socialConnectError, setSocialConnectError] = useState<{
    provider: "facebook" | "linkedin" | "googleads";
    message: string;
  } | null>(null);
  ...
  function startOauth(provider: "facebook" | "linkedin" | "googleads") {
    ...
    setSocialConnectError((prev) => (prev?.provider === provider ? null : prev));
  ...
  const refreshSocial = useCallback(async () => {
    ...
    try {
      const [fb, li, ga] = await Promise.all([
        fetch(`/api/facebook/status?session=${encodeURIComponent(sessionIdForOauth)}`).then(r => r.ok ? r.json() : { connected: false, label: null }).catch(() => ({ connected: false, label: null })),
        fetch(`/api/linkedin/status?session=${encodeURIComponent(sessionIdForOauth)}`).then(r => r.ok ? r.json() : { connected: false, label: null }).catch(() => ({ connected: false, label: null })),
        fetch(`/api/googleads/status?session=${encodeURIComponent(sessionIdForOauth)}`).then(r => r.ok ? r.json() : { connected: false, label: null }).catch(() => ({ connected: false, label: null })),
      ]);
      setFb(!!fb.connected); setLi(!!li.connected); setGa(!!ga.connected);
      setFbLabel(fb.label ?? null); setLiLabel(li.label ?? null); setGaLabel(ga.label ?? null);
    } finally {
      setSocialLoading(false);
    }
  }, [sessionIdForOauth]);

  const disconnectSocial = useCallback(async (provider: "facebook" | "linkedin" | "googleads") => {
    if (!token || socialDisconnecting) return;
    setSocialDisconnecting(provider);
    setSocialConnectError(null);
    try {
      await fetch("/api/onboarding/disconnectPlatform", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ onboardingToken: token, platform: provider }),
      });
      await refreshSocial();
    } finally {
      setSocialDisconnecting(null);
    }
  }, [token, socialDisconnecting, refreshSocial]);
  ...
  useEffect(() => {
    ...
    const onSocialConnected = (event: MessageEvent) => {
      ...
      // app/social-connect-failed posts this shape (not a bare string like the success pages)
      // when the popup couldn't complete — denied consent, an expired link, a failed token
      // exchange. Surfaces the failure inline instead of the button just silently staying
      // "Not Connected" with nothing telling the client anything went wrong.
      const data = event.data as { type?: string; provider?: string; reason?: string } | undefined;
      if (data?.type === "social-connect-failed" && (data.provider === "facebook" || data.provider === "linkedin" || data.provider === "googleads")) {
        try { socialWindowRefs.current[data.provider]?.close(); } catch {}
        socialWindowRefs.current[data.provider] = null;
        setSocialConnectError({ provider: data.provider, message: SOCIAL_FAILURE_MESSAGES[data.reason || ""] || "The connection could not be completed." });
      }
    };
  ...
  useEffect(() => {
    async function fetchSession() {
      ...
      try {
        ...
        // Resume on the furthest page reached, not always module 1 — otherwise a returning
        // client sees their real progress in the sidebar but still has to manually click back
        // to where they left off.
        setStep((prev) => Math.max(prev, inferredMax));
      } catch {
              ...
              {q.type === "video" && (!isVideo3 || video3SubStep === 0) && (!isVideo4 || video4SubStep === 0) ? (
                ...
                <div className="rounded-2xl border border-[#E5E7EB] bg-white p-6 lg:p-10 mb-8 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6">
                    {[
                      { provider: "linkedin" as const, label: "LinkedIn", connected: liConnected, accountLabel: liLabel, color: "bg-[#0A66C2]", icon: "M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z", disabled: false },
                      // Facebook OAuth is temporarily disabled onboarding-side — leave the tile
                      // visible (so returning clients don't lose the row layout) but unclickable.
                      { provider: "facebook" as const, label: "Facebook", connected: fbConnected, accountLabel: fbLabel, color: "bg-[#1877F2]", icon: "M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z", disabled: true },
                      { provider: "googleads" as const, label: "Google Ads", connected: gaConnected, accountLabel: gaLabel, color: "bg-[#EA4335]", icon: "", disabled: false },
                    ].map(({ provider, label, connected, accountLabel, color, icon, disabled }) => (
                      <div key={provider} className={`flex flex-col items-center p-5 lg:p-6 bg-[#FAFAFA] rounded-xl border border-[#E5E7EB] ${disabled ? "opacity-50" : ""}`}>
                        <button
                          type="button"
                        ...
                        >
                          <div className={`w-12 h-12 ${color} rounded-xl mb-3 flex items-center justify-center shadow-sm transition-transform ${connected || disabled ? "" : "group-hover:scale-105"}`}>
                            {icon ? (
                          ...
                          <span className={`text-xs mt-1.5 font-medium ${connected ? "text-green-600" : "text-[#9CA3AF]"} ${inter.className}`}>
                            {disabled ? "Coming Soon" : connected ? (accountLabel ? `Connected as ${accountLabel}` : "Connected") : "Not Connected"}
                          </span>
                        </button>
                        {connected && !disabled && (
                          <button
                            ...
                            disabled={socialDisconnecting === provider}
                            className={`mt-2 text-xs font-medium text-[#9CA3AF] hover:text-red-600 transition disabled:pointer-events-none ${inter.className}`}
                          >
                            {socialDisconnecting === provider ? "Disconnecting..." : "Disconnect"}
                          </button>
                        )}
                        {!disabled && socialConnectError?.provider === provider && (
                          <p className={`mt-2 text-xs text-red-600 text-center ${inter.className}`}>{socialConnectError.message}</p>
                        )}
                      </div>
                    ))}
                  </div>
