...
const SignPdfModal: React.FC<SignPdfModalProps> = ({ isOpen, onClose }) => {
  ...
  const [signatureCoords, setSignatureCoords] = useState<SignatureCoords | null>(null);
  ...
  useEffect(() => {
    if (isPdfLoading) {
      ...
      loadTimeoutRef.current = setTimeout(() => {
        setError('PDF loading timed out. The file might be too large, corrupted, or there could be network issues.');
  ...
  const handlePdfLoadError = (err: Error) => {
    ...
    if (err.name === 'WorkerException') {
      message += ' The PDF worker failed. Please check your network connection or try reloading the page.';
      ...
      message = 'Failed to load PDF: The file appears to be invalid or corrupted.';
    }
    setError(message);
    setIsPdfLoading(false);
  };
  ...
  const handleGenerateLink = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to generate signing link.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 className={`text-2xl font-bold mb-5 text-center ${bricolage.className}`}>Preparing The PDF</h2>
        ...
        <div className="flex-grow overflow-y-auto px-2 pb-4">
          {!pdfFile ? (
            ...
            <div className="w-full flex flex-col items-center">
              ...
              <p className="mb-4 text-sm text-gray-300">Click on the PDF page below to place the signature box.</p>
              <div className="pdf-container border border-gray-500 mb-4 relative bg-white w-full">
                <Document
                  ...
                  loading={
                    isPdfLoading ? <div className="text-black p-4 text-center">Loading PDF...</div> : null
                  }
                >
                  {!isPdfLoading && !error?.startsWith('Failed to load PDF') && (
                    <div ref={pageRef} onClick={handlePageClick} style={{ position: 'relative', cursor: 'crosshair', display: 'inline-block' }}>
                      <Page
                        pageNumber={currentPage}
                        renderTextLayer={false}
                        onLoadSuccess={onPageLoadSuccess}
                        onRenderError={(err) => setError(`Failed to render page ${currentPage}: ${err.message}`)}
                      />
                      {signatureCoords && signatureCoords.page === currentPage && pageRef.current && (
                        <div
                          style={{
                            position: 'absolute',
                            left: `${signatureCoords.xRatio * pageRef.current.clientWidth}px`,
                            top: `${signatureCoords.yRatio * pageRef.current.clientHeight}px`,
              ...
              {error && (error.startsWith('Failed to load PDF') || error.startsWith('Failed to render page') || error.startsWith('PDF loading timed out')) && (
                 <p className="text-red-500 my-4 text-center font-semibold">{error}</p>
              )}

              {!isPdfLoading && numPages && !error?.startsWith('Failed to load PDF') && (
                <>
                  {numPages > 1 && (
                  ...
                  {signatureCoords && (
                    <p className="mb-4 text-sm text-green-400">
                      The signature spot has been selected on page {signatureCoords.page}. Click on the PDF again to change.
                    </p>
                  )}
                  ...
                  {generatedLink && (
                    <div className="mt-6 p-4 bg-[#2a0f4a]/70 border border-[#701CC0]/60 rounded-lg w-full max-w-lg text-center shadow-md">
                      <p className="text-white mb-2 font-semibold">Link Generated:</p>
                      <input
                        type="text"
                        readOnly
                        value={generatedLink}
