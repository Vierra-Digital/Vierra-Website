...
import { getBusy, getBusyOverRange, resolveVisibleCalendarIds, type BusyInterval } from "@/lib/calendar/googleCalendar";
  ...
  const perMemberBusy = (
    await Promise.all(
      members.map(async ({ user_id }) => {
        ...
        const calendarIds = await resolveVisibleCalendarIds(user_id, email, token.accessToken);
        return getBusyOverRange(token.accessToken, rangeStartIso, rangeEndIso, calendarIds);
      })
    )
...
export async function findFreeCompanyMembers(companyId: string, startIso: string, endIso: string): Promise<string[]> {
  ...
  const results = await Promise.all(
    members.map(async ({ user_id }) => {
      ...
      const calendarIds = await resolveVisibleCalendarIds(user_id, email, token.accessToken);
      const busy = await getBusy(token.accessToken, startIso, endIso, calendarIds);
