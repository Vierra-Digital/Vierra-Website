...
// A real UUID, not a short placeholder — companyId is validated as one (see
// lib/api/targetCompany.ts's explicitCompanyIdFromRequest) so a malformed value can't reach
// Postgres and throw P2007 instead of a clean 400.
const COMPANY_ID = "11111111-1111-4111-8111-111111111111";

const call = (body: unknown, query: Record<string, unknown> = { companyId: COMPANY_ID }) => {
  const res = mockRes();
  return handler(
...
describe("access", () => {
  it("takes DELETE and nothing else", async () => {
    const res = mockRes();
    await handler(
      { method: "POST", query: { companyId: COMPANY_ID }, headers: {}, body: {} } as never,
      res as never
    );
...
describe("mailboxes", () => {
  it("matches on the client's company, not on who is asking", async () => {
    ...
    expect(mailboxDeleteMany).toHaveBeenCalledWith({
      where: { company_id: COMPANY_ID, account_email: "desk@acme.com" },
    });
  });
